...
function configure(parser)
	...
	parser:option("-r --rate", "Transmit rate in Mbit/s."):default(140):convert(tonumber)
...
function loadSlave(queue, rxDev, size, flows, offset)
	...
	while mg.running() do
		...
		for i, buf in ipairs(bufs) do
			...
			if size_cnt % 16 < 4 then
				buf:setSize(200) -- sets the buffer size
				pkt:setLength(200)
			elseif size_cnt % 16 < 8 then
				buf:setSize(600) -- sets the buffer size
				pkt:setLength(600)
			elseif size_cnt % 16 < 12 then
				buf:setSize(800) -- sets the buffer size
				pkt:setLength(800)
			elseif size_cnt % 16 < 16 then
				buf:setSize(1200) -- sets the buffer size
				pkt:setLength(1200)
			end
			size_cnt = size_cnt + 1
		end
